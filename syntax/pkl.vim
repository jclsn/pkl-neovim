...
exe $'syn match	pklStruct        "\<typealias\>\s*{s:id}*" contains=pklType'
exe $'syn match	pklStruct        "\<class\>\n*\s*{s:id}*" contains=pklType'

" --- Types ---
exe $'syn match	pklType "\<typealias\>\s\+\zs{s:id}\+" contained contains=ALLBUT,pklIdentifier,pklUnicode'
...
exe $'syn region	pklParameterizedType start="{s:id}\zs\s*<" end=">" keepend extend contains=ALLBUT,pklIdentifier,pklUnicode'
...
execute $'syn match pklObjectDeclaration ":\s*\zs\%(\s*<*\*\?{s:qid}>*\s*|\?\s*\)\+"' .
      \ ' contains=pklType,pklParameterizedType,pklOperator,pklParen,pklKeyword'
...
exe $'syn match	pklFunction	"\<\h{s:id}*\>\ze\s*[?|\*]\?("'
